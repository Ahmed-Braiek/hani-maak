...
type VoiceState="idle"|"connecting"|"live"|"fallback";
type ConversationLocale="ar"|"fr"|"en";
type RecognitionConstructor=new()=>any;

function recognitionConstructor():RecognitionConstructor|null{if(typeof window==="undefined")return null;const w=window as typeof window&{SpeechRecognition?:RecognitionConstructor;webkitSpeechRecognition?:RecognitionConstructor};return w.SpeechRecognition??w.webkitSpeechRecognition??null}
...
export function HeniCompanion(){
  ...
  const [open,setOpen]=useState(false);const [sessionId,setSessionId]=useState("");const [confirmationToken,setConfirmationToken]=useState("");const [conversationLocale,setConversationLocale]=useState<ConversationLocale>(locale);const [messages,setMessages]=useState<Message[]>([{role:"heni",text:tr("heni.hello")}]);const [input,setInput]=useState("");const [busy,setBusy]=useState(false);const [speaking,setSpeaking]=useState(false);const [voiceState,setVoiceState]=useState<VoiceState>("idle");const [fallbackListening,setFallbackListening]=useState(false);const [micSupported,setMicSupported]=useState(true);
  const endRef=useRef<HTMLDivElement>(null);const mountedRef=useRef(true);const busyRef=useRef(false);const wsRef=useRef<WebSocket|null>(null);const recognitionRef=useRef<any>(null);const fallbackRestartRef=useRef(false);const streamRef=useRef<MediaStream|null>(null);const audioContextRef=useRef<AudioContext|null>(null);const sourceNodeRef=useRef<MediaStreamAudioSourceNode|null>(null);const processorRef=useRef<ScriptProcessorNode|null>(null);const silentGainRef=useRef<GainNode|null>(null);const playbackSourcesRef=useRef(new Set<AudioBufferSourceNode>());const nextPlaybackRef=useRef(0);const lastTranscriptRoleRef=useRef<"user"|"heni"|null>(null);const liveToolRef=useRef<string|undefined>(undefined);
  const listening=voiceState==="live"||fallbackListening;
  const liveLabel=locale==="ar"?"محادثة صوتية مباشرة":locale==="en"?"Live voice":"Voix en direct";
  const fallbackLabel=locale==="ar"?"محادثة صوتية":locale==="en"?"Voice conversation":"Conversation vocale";
  const fallbackNotice=locale==="ar"?"هاني معاك بالصوت. احكي عادي، ونكمل معاك المحادثة.":locale==="en"?"Heni is listening. Speak naturally and we will keep the voice conversation going.":"Heni vous écoute. Parlez naturellement, la conversation vocale continue.";
  ...
  const cleanupVoice=useCallback((closeSocket=true)=>{
    ...
    streamRef.current?.getTracks().forEach(track=>track.stop());streamRef.current=null;recognitionRef.current?.abort?.();recognitionRef.current?.stop?.();recognitionRef.current=null;fallbackRestartRef.current=false;if(mountedRef.current)setFallbackListening(false);stopPlayback();
  ...
  useEffect(()=>{mountedRef.current=true;setMicSupported(Boolean(typeof window!=="undefined"&&navigator.mediaDevices&&(("WebSocket" in window&&"AudioContext" in window)||recognitionConstructor())));return()=>{mountedRef.current=false;cleanupVoice(true);if(typeof window!=="undefined"&&"speechSynthesis" in window)window.speechSynthesis.cancel()}},[cleanupVoice]);
  useEffect(()=>{setSessionId("");setConfirmationToken("");setConversationLocale(locale);setMessages([{role:"heni",text:tx(locale,"heni.hello")}]);setInput("");cleanupVoice(true)},[locale,cleanupVoice]);
  useEffect(()=>{if(open)endRef.current?.scrollIntoView({behavior:"smooth",block:"nearest"})},[messages,open,busy,voiceState]);

  function speak(text:string){if(typeof window==="undefined"||!("speechSynthesis" in window)||(voiceState!=="idle"&&voiceState!=="fallback"))return;window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(text);utterance.lang=conversationLocale==="ar"?"ar-TN":conversationLocale==="fr"?"fr-FR":"en-GB";utterance.rate=.93;const voices=window.speechSynthesis.getVoices();const masculineNames=conversationLocale==="ar"?["Majed","Maged","Tarik","Hamed"]:conversationLocale==="fr"?["Thomas","Henri","Paul","Nicolas"]:["Daniel","Alex","George","David","Mark"];const languageVoices=voices.filter(v=>v.lang.toLowerCase().startsWith(utterance.lang.toLowerCase().slice(0,2)));const wanted=masculineNames.map(name=>languageVoices.find(v=>v.name.toLowerCase().includes(name.toLowerCase()))).find(Boolean)??languageVoices.find(v=>/male|masculin|man/i.test(v.name))??languageVoices[0];if(wanted)utterance.voice=wanted;utterance.onstart=()=>mountedRef.current&&setSpeaking(true);utterance.onend=()=>{if(mountedRef.current)setSpeaking(false);if(fallbackRestartRef.current)window.setTimeout(()=>void startFallbackRecognition(),260)};utterance.onerror=()=>{if(mountedRef.current)setSpeaking(false);if(fallbackRestartRef.current)window.setTimeout(()=>void startFallbackRecognition(),260)};window.speechSynthesis.speak(utterance)}

  const send=useCallback(async(rawText?:string)=>{const text=(rawText??input).trim();if(!text||busyRef.current)return;busyRef.current=true;setBusy(true);setInput("");setMessages(current=>[...current,{role:"user",text}]);try{const history=messages.slice(-10).map(message=>({role:message.role==="user"?"user":"assistant",content:message.text}));const res=await fetch("/api/v1/heni/chat",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({message:text,locale:conversationLocale,patientId:"patient-amal",role:"patient",source:"floating_heni",sessionId:sessionId||undefined,confirmationToken:confirmationToken||undefined,history})});const payload=await safeJson(res);if(!res.ok)throw new Error(String(payload?.error||"heni_chat_failed"));if(payload.sessionId&&mountedRef.current)setSessionId(String(payload.sessionId));if(mountedRef.current&&["ar","fr","en"].includes(String(payload.locale)))setConversationLocale(payload.locale as ConversationLocale);if(mountedRef.current)setConfirmationToken(typeof payload.confirmationToken==="string"?payload.confirmationToken:"");const answer=String(payload.message??tx(conversationLocale,"heni.chat_error"));if(mountedRef.current)setMessages(current=>[...current,{role:"heni",text:answer,tool:payload.tool}]);speak(answer)}catch{if(mountedRef.current)setMessages(current=>[...current,{role:"heni",text:tx(locale,"heni.chat_error")}])}finally{busyRef.current=false;if(mountedRef.current)setBusy(false)}},[input,conversationLocale,messages,sessionId,confirmationToken,voiceState]);

  async function startFallbackRecognition(){
    if(!fallbackRestartRef.current||busyRef.current||recognitionRef.current)return;
    const Recognition=recognitionConstructor();
    if(!Recognition){fallbackRestartRef.current=false;if(mountedRef.current){setFallbackListening(false);setVoiceState("idle");setMessages(current=>[...current,{role:"heni",text:liveError}])}return}
    try{
      const recognition=new Recognition();recognitionRef.current=recognition;recognition.lang=conversationLocale==="ar"?"ar-TN":conversationLocale==="fr"?"fr-FR":"en-GB";recognition.interimResults=true;recognition.continuous=false;recognition.maxAlternatives=1;
      recognition.onstart=()=>mountedRef.current&&setFallbackListening(true);
      recognition.onend=()=>{recognitionRef.current=null;if(mountedRef.current)setFallbackListening(false);if(fallbackRestartRef.current&&!busyRef.current&&!speaking)window.setTimeout(()=>void startFallbackRecognition(),320)};
      recognition.onerror=(event:any)=>{recognitionRef.current=null;if(mountedRef.current)setFallbackListening(false);if(event?.error==="not-allowed"||event?.error==="service-not-allowed"){fallbackRestartRef.current=false;if(mountedRef.current){setVoiceState("idle");setMessages(current=>[...current,{role:"heni",text:tr("heni.mic_denied")}])}}};
      recognition.onresult=(event:any)=>{const result=event.results?.[event.results.length-1];const transcript=String(result?.[0]?.transcript??"").trim();if(!transcript)return;if(result.isFinal){try{recognition.stop()}catch{}void send(transcript)}};
      recognition.start();
    }catch{recognitionRef.current=null;if(mountedRef.current)setFallbackListening(false);if(fallbackRestartRef.current)window.setTimeout(()=>void startFallbackRecognition(),500)}
  }

  async function activateFallbackVoice(){
    const Recognition=recognitionConstructor();
    if(!Recognition){if(mountedRef.current){setVoiceState("idle");setMessages(current=>[...current,{role:"heni",text:liveError}])}return}
    fallbackRestartRef.current=true;if(mountedRef.current){setVoiceState("fallback");setMessages(current=>{const last=current[current.length-1];return last?.text===fallbackNotice?current:[...current,{role:"heni",text:fallbackNotice}]})}
    await startFallbackRecognition();
  }
  ...
  async function startLiveVoice(){
    ...
    try{
      ...
      const tokenResponse=await fetch("/api/v1/heni/voice-token",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({locale:conversationLocale}),cache:"no-store"});const tokenPayload=await safeJson(tokenResponse);if(!tokenResponse.ok||!tokenPayload.token||!tokenPayload.wsUrl)throw new Error("voice_token_failed");
      ...
      ws.onclose=()=>{if(wsRef.current===ws){cleanupVoice(false);void activateFallbackVoice()}};ws.onerror=()=>{};
      setVoiceState("live");
    }catch{cleanupVoice(true);await activateFallbackVoice()}
  }

  async function toggleLiveVoice(){if(voiceState!=="idle"){cleanupVoice(true);return}await startLiveVoice()}
  ...
  return <div className={`heni-companion ${open?"is-open":""} ${rtl?"rtl":""}`} dir={rtl?"rtl":"ltr"}>
    {open&&<section className="heni-panel" aria-label={tr("heni.title")}>
      <header className="heni-panel-head"><div className="row-start"><HeniAvatar size={58} speaking={speaking} listening={listening}/><div><div className="eyebrow">Heni · هاني</div><strong>{tr("heni.title")}</strong><div className="heni-online"><span/> {voiceState==="connecting"?connectingLabel:voiceState==="live"?liveLabel:voiceState==="fallback"?fallbackLabel:tr("heni.online")}</div></div></div><button className="icon-btn" onClick={()=>{setOpen(false);cleanupVoice(true)}} aria-label={tr("heni.close")}>×</button></header>
      <div className="heni-chat-log" aria-live="polite">{messages.map((m,i)=><div className={`heni-message ${m.role}`} key={`${m.role}-${i}`} dir="auto">{m.text}{m.tool&&<div className="heni-tool">✓ {m.tool}</div>}</div>)}{busy&&<div className="heni-thinking"><span/><span/><span/></div>}{voiceState==="connecting"&&<div className="heni-recording-state"><span/> {connectingLabel}</div>}{voiceState==="live"&&<div className="heni-recording-state"><span/> {liveLabel}</div>}{voiceState==="fallback"&&<div className="heni-recording-state"><span/> {fallbackListening?fallbackLabel:(locale==="ar"?"هاني يحكي…":locale==="en"?"Heni is speaking…":"Heni parle…")}</div>}<div ref={endRef}/></div>
    ...
    <button className="heni-fab" onClick={()=>setOpen(v=>!v)} aria-expanded={open} aria-label={tr("heni.open")}><HeniAvatar size={74} speaking={speaking} listening={listening}/>{!open&&<span className="heni-fab-label"><strong>Heni</strong><small>{voiceState==="live"?liveLabel:voiceState==="fallback"?fallbackLabel:tr("heni.with_you")}</small></span>}</button>
  </div>;
}
