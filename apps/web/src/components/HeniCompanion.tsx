...
type UiAction={kind?:string;label?:string;url?:string};
type Message={role:"heni"|"user";text:string;tool?:string};
type VoiceState="idle"|"connecting"|"live";
type ConversationLocale="ar"|"fr"|"en";
type VoiceControl={type?:string;sessionId?:string;role?:"user"|"model";text?:string;name?:string;message?:string;locale?:ConversationLocale;action?:UiAction};
...
export function HeniCompanion(){
  ...
  const [open,setOpen]=useState(false);const [sessionId,setSessionId]=useState("");const [confirmationToken,setConfirmationToken]=useState("");const [conversationLocale,setConversationLocale]=useState<ConversationLocale>(locale);const [uiActions,setUiActions]=useState<UiAction[]>([]);const [messages,setMessages]=useState<Message[]>([{role:"heni",text:tr("heni.hello")}]);const [input,setInput]=useState("");const [busy,setBusy]=useState(false);const [speaking,setSpeaking]=useState(false);const [voiceState,setVoiceState]=useState<VoiceState>("idle");const [micSupported,setMicSupported]=useState(true);
  ...
  useEffect(()=>{setSessionId("");setConfirmationToken("");setConversationLocale(locale);setUiActions([]);setMessages([{role:"heni",text:tx(locale,"heni.hello")}]);setInput("");cleanupVoice(true)},[locale,cleanupVoice]);
  ...
  async function startLiveVoice(){
    ...
    try{
      ...
      ws.onmessage=event=>{if(event.data instanceof ArrayBuffer){playPcm(event.data);return}if(typeof event.data!=="string")return;try{const message=JSON.parse(event.data) as VoiceControl;if(message.type==="session"&&message.sessionId)setSessionId(message.sessionId);else if(message.type==="locale"&&message.locale)setConversationLocale(message.locale);else if(message.type==="transcript"&&message.text)appendTranscript(message.role==="user"?"user":"heni",message.text);else if(message.type==="tool_call"&&message.name)liveToolRef.current=message.name;else if(message.type==="ui_action"&&message.action?.url&&message.action?.label)setUiActions(current=>[...current.filter(action=>action.url!==message.action?.url),message.action as UiAction].slice(-3));else if(message.type==="turn_complete"){lastTranscriptRoleRef.current=null;liveToolRef.current=undefined}else if(message.type==="interrupted"){stopPlayback();lastTranscriptRoleRef.current=null}else if(message.type==="error")setMessages(current=>[...current,{role:"heni",text:liveError}])}catch{}};
  ...
  return <div className={`heni-companion ${open?"is-open":""} ${rtl?"rtl":""}`} dir={rtl?"rtl":"ltr"}>
    {open&&<section className="heni-panel" aria-label={tr("heni.title")}>
      ...
      <div className="heni-chat-log" aria-live="polite">{messages.map((m,i)=><div className={`heni-message ${m.role}`} key={`${m.role}-${i}`} dir="auto">{m.text}{m.tool&&<div className="heni-tool">✓ {m.tool}</div>}</div>)}{busy&&<div className="heni-thinking"><span/><span/><span/></div>}{voiceState==="connecting"&&<div className="heni-recording-state"><span/> {connectingLabel}</div>}{listening&&<div className="heni-recording-state"><span/> {liveLabel}</div>}<div ref={endRef}/></div>
      {uiActions.length>0&&<div className="heni-agent-actions">{uiActions.map((action,index)=><a key={`${action.url}-${index}`} href={action.url} target="_blank" rel="noreferrer noopener"><span>↗</span><strong>{action.label}</strong></a>)}</div>}<div className="heni-quick-row">{prompts.map(prompt=><button key={prompt} onClick={()=>void send(prompt)} disabled={busy||voiceState!=="idle"}>{prompt}</button>)}</div>
