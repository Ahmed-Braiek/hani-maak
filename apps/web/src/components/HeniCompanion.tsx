...
export function HeniCompanion(){
  ...
  const [open,setOpen]=useState(false);const [sessionId,setSessionId]=useState("");const [confirmationToken,setConfirmationToken]=useState("");const [conversationLocale,setConversationLocale]=useState<ConversationLocale>(locale);const [messages,setMessages]=useState<Message[]>([{role:"heni",text:tr("heni.hello")}]);const [input,setInput]=useState("");const [busy,setBusy]=useState(false);const [speaking,setSpeaking]=useState(false);const [voiceState,setVoiceState]=useState<VoiceState>("idle");const [micSupported,setMicSupported]=useState(true);
