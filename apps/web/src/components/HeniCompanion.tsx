...
export function HeniCompanion(){
  ...
  const send=useCallback(async(rawText?:string)=>{const text=(rawText??input).trim();if(!text||busyRef.current)return;busyRef.current=true;setBusy(true);setInput("");setMessages(current=>[...current,{role:"user",text}]);try{const history=messages.slice(-10).map(message=>({role:message.role==="user"?"user":"assistant",content:message.text}));const res=await fetch("/api/v1/heni/chat",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({message:text,locale,patientId:"patient-amal",role:"patient",source:"floating_heni",sessionId:sessionId||undefined,history})});const payload=await safeJson(res);if(!res.ok)throw new Error(String(payload?.error||"heni_chat_failed"));if(payload.sessionId&&mountedRef.current)setSessionId(String(payload.sessionId));const answer=String(payload.message??tx(locale,"heni.chat_error"));if(mountedRef.current)setMessages(current=>[...current,{role:"heni",text:answer,tool:payload.tool}]);speak(answer)}catch{if(mountedRef.current)setMessages(current=>[...current,{role:"heni",text:tx(locale,"heni.chat_error")}])}finally{busyRef.current=false;if(mountedRef.current)setBusy(false)}},[input,locale,messages,sessionId,voiceState]);
