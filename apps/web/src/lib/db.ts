...
function persistentStorageError(){return new Error("Persistent storage is not configured for this deployment. Configure NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SECRET_KEY (or legacy SUPABASE_SERVICE_ROLE_KEY).")}
...
function supabaseHeaders(extra:Record<string,string>={}){if(!supabaseKey)throw persistentStorageError();const headers:Record<string,string>={apikey:supabaseKey,"Content-Type":"application/json"};if(!supabaseKey.startsWith("sb_secret_")&&!supabaseKey.startsWith("sb_publishable_"))headers.Authorization=`Bearer ${supabaseKey}`;return{...headers,...extra}}
