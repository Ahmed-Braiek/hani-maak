...
export function detectClinicalBoundary(text:string){
  const x=norm(text);
  return [
    "dose","dosage","augmenter","زيد الجرعة","نزيد الجرعة","نقص الجرعة","diagnostic","diagnosis","diagnose",
    "تشخيص","prescrire","prescribe","وصفة","urgence","استعجالي","safe to take","can i take","نجم ناخذ",
    "interaction médicament","allergie","pregnant","حامل"
  ].some(k=>x.includes(norm(k)));
}
