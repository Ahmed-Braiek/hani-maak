...
const scenes=[
  ...
  ["03","Recorded AR walkthrough","Walk the real hospital video toward the Médecine nucléaire block while Heni speaks and a moving target guides the route.","/patient/map/ar?demo=nuclear-medicine","Recorded route + voice"],
  ...
  ["05","Auditable AI","Open the staff trace with the correct Super Admin demo role already selected.","/api/v1/staff/session/launch?role=super_admin&next=/staff/calls","MCP/tool proof"],
...
export default function Present(){return <div className="container presentation-page" style={{padding:"24px 0 90px"}}>
  <div className="topbar presentation-topbar"><Logo/><div className="nav-actions"><Link className="btn btn-secondary" href="/">⌂ Home</Link><Link className="btn btn-secondary" href="/staff">▣ Staff</Link><Link className="btn btn-primary" href="/patient/map/ar?demo=nuclear-medicine">▶ AR demo</Link></div></div>
  <div className="presentation-hero" style={{padding:"68px 0 28px"}}>
    ...
    <p className="muted" style={{fontSize:"1.08rem",maxWidth:800,lineHeight:1.7}}>Every card now opens a working surface with the required demo role. For the strongest live sequence: booking → recorded AR walk → Heni voice → staff trace → continuity.</p>
  </div>
  ...
  <div className="notice info" style={{marginTop:18}}><strong>Truthfulness note for the jury:</strong> the walkthrough video was provided by the project team. The Heni target, voice timing and indoor guidance overlay are a prototype demonstration and are not validated indoor positioning.</div>
</div>}
