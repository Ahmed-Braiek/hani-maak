...
export default function Landing(){return <>
  <section className="hero competition-hero"><div className="container">
    <div className="topbar"><Logo/><div className="nav-actions"><Link className="btn btn-secondary" href="/present">Presentation mode</Link><Link className="btn btn-secondary" href="/staff">Staff console</Link><Link className="btn btn-primary" href="/patient">Start demo →</Link></div></div>
    <div className="hero-grid">
      <div className="hero-copy">
        <div className="eyebrow">Future Health Connectathon · Tunis 2026</div>
  ...
  <section className="section section-soft"><div className="container"><div className="eyebrow">One journey · multiple interfaces</div><h2 className="h2">Not another appointment app.</h2><div className="grid-3"><article className="feature"><div className="feature-icon">◷</div><h3>Access</h3><p>Rule-derived slots, confirmation, cancellation and rescheduling through mobile or conversation.</p></article><article className="feature"><div className="feature-icon">⌖</div><h3>Guidance</h3><p>Preparation, deterministic route graph and a camera-based AR demo that accompanies the patient toward the service.</p></article><article className="feature"><div className="feature-icon">✓</div><h3>Continuity</h3><p>Provider-authored instructions, reminders, next steps and staff visibility after the appointment.</p></article></div></div></section>

  <section className="section"><div className="container"><div className="proof-strip"><div><span>01</span><strong>Patient books</strong><small>real domain state</small></div><i>→</i><div><span>02</span><strong>Heni guides</strong><small>map + AR</small></div><i>→</i><div><span>03</span><strong>Voice acts</strong><small>constrained tools</small></div><i>→</i><div><span>04</span><strong>Staff sees it</strong><small>auditable timeline</small></div><i>→</i><div><span>05</span><strong>Journey continues</strong><small>follow-up</small></div></div><div className="demo-band" style={{marginTop:22}}><div><div className="eyebrow" style={{color:"#8de0d3"}}>Competition scenario</div><h2 style={{margin:"7px 0"}}>One patient journey. One shared operational truth.</h2><div style={{color:"#bad5d0"}}>Book on mobile → walk the route → talk in Tunisian Arabic → inspect the same action in operations.</div></div><Link className="btn" style={{background:"white",color:"#143d38"}} href="/present">Run presentation →</Link></div></div></section>
  <footer className="footer">Hani Maak · synthetic competition environment · not a clinical decision system</footer>
</>}
